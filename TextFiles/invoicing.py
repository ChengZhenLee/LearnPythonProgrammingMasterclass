...
def next_invoice_number(invoice_number: str) -> str:
    ...
    """
    invoice_year, current_number = parse_invoice_number(invoice_number)
    year = get_year()

    # If the year isn't equal
    if year != invoice_year:
        # Increment the year and set the next_number
        invoice_year = year
        next_number = 1
    else:
    ...
    next_invoice_number = f'{invoice_year}-{next_number:04d}'
...
def record_invoice(invoice_file: TextIO,
    ...
    """
    # Move the file pointer to the last line
    invoice_file.seek(last_line_ptr, SEEK_SET)
    last_row = ""
    # Loop through the file from the pointer and check if it's empty
    for line in invoice_file:
        print('*', end='')  # TODO delete after testing
        last_row = line
    # If the file is not empty
    if last_row:
        # Use '_' for values/variables that won't be used
    ...
    # Save the pointer position before writing
